...
from batchgenerators_local.augmentations.spatial_transformations import augment_spatial, augment_spatial_2, \
...
class MirrorTransform(AbstractTransform):
    ...
    def __init__(self, axes=(0, 1, 2), data_key="data", label_key="seg", dist_key="distmap"):
        self.data_key = data_key
        self.label_key = label_key
    ...
    def __call__(self, **data_dict):
        data = data_dict.get(self.data_key)
        seg = data_dict.get(self.label_key)
        dist = data_dict.get(self.dist_key)

        for b in range(len(data)):
            sample_seg = None
            if seg is not None:
                sample_seg = seg[b]
            ret_val = augment_mirroring(data[b], sample_seg, dist[b], axes=self.axes)
            data[b] = ret_val[0]
            dist[b] = ret_val[2]
            if seg is not None:
                seg[b] = ret_val[1]
...
class SpatialTransform(AbstractTransform):
    ...
    def __init__(self, patch_size, patch_center_dist_from_border=30,
                 ...
                 label_key="seg", dist_key="distmap", p_el_per_sample=1, p_scale_per_sample=1, p_rot_per_sample=1,
    ...
    def __call__(self, **data_dict):
        ...
        data_dict[self.data_key] = ret_val[0]
        data_dict[self.dist_key] = ret_val[2]
        if seg is not None:
