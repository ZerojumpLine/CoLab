...
import numpy as np
from batchgenerators_local.augmentations.utils import pad_nd_image


def center_crop(data, dist, crop_size, seg=None):
    return crop(data, seg, dist, crop_size, 0, 'center')
...
def crop(data, seg=None, dist=None, crop_size=128, margins=(0, 0, 0), crop_type="center",
    ...
    dist_shape = tuple([len(dist)] + list(dist[0].shape))
    dist_dtype = dist[0].dtype
    ...
    dist_return = np.zeros([dist_shape[0], dist_shape[1]] + list(crop_size), dtype=dist_dtype)
    if seg is not None:
    ...
    for b in range(data_shape[0]):
        ...
        dist_shape_here = [dist_shape[0]] + list(dist[b].shape)
        if seg is not None:
        ...
        slicer_dist = [slice(0, dist_shape_here[1])] + [slice(lbs[d], ubs[d]) for d in range(dim)]
        dist_cropped = dist[b][tuple(slicer_dist)]
        ...
        if any([i > 0 for j in need_to_pad for i in j]):
            ...
            dist_return[b] = np.pad(dist_cropped, need_to_pad, pad_mode, **pad_kwargs)
            if seg_return is not None:
                seg_return[b] = np.pad(seg_cropped, need_to_pad, pad_mode_seg, **pad_kwargs_seg)
        else:
            data_return[b] = data_cropped
            dist_return[b] = dist_cropped
            if seg_return is not None:
                seg_return[b] = seg_cropped

    return data_return, seg_return, dist_return


def random_crop(data, seg=None, dist=None, crop_size=128, margins=[0, 0, 0]):
    return crop(data, seg, dist, crop_size, margins, 'random')
