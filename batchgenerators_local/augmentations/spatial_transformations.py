...
import numpy as np
from batchgenerators_local.augmentations.utils import create_zero_centered_coordinate_mesh, elastic_deform_coordinates, \
    interpolate_img, \
    rotate_coords_2d, rotate_coords_3d, scale_coords, resize_segmentation, resize_multichannel_image, \
    elastic_deform_coordinates_2
from batchgenerators_local.augmentations.crop_and_pad_augmentations import random_crop as random_crop_aug
from batchgenerators_local.augmentations.crop_and_pad_augmentations import center_crop as center_crop_aug
    ...
    for sample_id in range(data.shape[0]):
        ...
        if modified_coords:
            ...
            for channel_id in range(dist.shape[1]):
                dist_result[sample_id, channel_id] = interpolate_img(dist[sample_id, channel_id], coords, 0,
                                                                     'nearest', cval=border_cval_data)
            ...
            dt = dist[sample_id:sample_id + 1]
            if random_crop:
                margin = [patch_center_dist_from_border[d] - patch_size[d] // 2 for d in range(dim)]
                d, s, dt = random_crop_aug(data[sample_id:sample_id + 1], s, dt, patch_size, margin)
            else:
                d, s, dt = center_crop_aug(data[sample_id:sample_id + 1], dt, patch_size, s)
            data_result[sample_id] = d[0]
            dist_result[sample_id] = dt[0]
            if seg is not None:
